...
maximize (f:fs) = foldl fmaxabs f fs

fmaxabs f g = \x -> if abs (f x) > abs (g x) then f x else g x

-- По-лесен вариант - без сравнение по абсолютна стойност
maximize' [] = error "empty list"
maximize' (f:fs) = foldl fmax f fs
