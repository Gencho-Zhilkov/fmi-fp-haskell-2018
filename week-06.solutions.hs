...
filterList _ []     = []
filterList f (x:xs) = if f x then x:fxs else fxs where
    fxs = filterList f xs
...
-}
subsets :: [a] -> [[a]]
subsets []       = [[]] -- единственото подсписък на празен списък е самият той
subsets (x:xs)   = map (x:) sxs ++ sxs where
    sxs = subsets xs
...
pick :: Int -> [a] -> [[a]]
pick k xs
    | k == 0        = [[]] -- единственият начин да избера 0 елемента е празния списък
    | length xs < k = []   -- не мога да взема повече елементи, отколкото има в списъка
    -- мога да избера k елемента от (h:ts) по два начина:
    -- или да взема първия - h - и да избера (k-1) от останалите - ts
    -- или да избера k от ts
    | otherwise     = map (h:) (pick (k - 1) ts) ++ pick k ts where
        h  = head xs
        ts = tail xs
