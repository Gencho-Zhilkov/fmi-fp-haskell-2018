...
-- филтрираме първо простите индекси и към тях добавяме не-простите
primeReorder :: [a] -> [a]
...
Примери:
    ...
    countMin [] = 0
-}
-- Кратко решение с filter, но с две обхождания на списъка
countMin :: [Integer] -> Int
countMin xs = length (filter (== min) xs) where min = minimum xs

-- Подобно на първото, но с list comprehension и sum
countMin' :: [Integer] -> Int
countMin' xs = sum [1 | x <- xs, x == min] where min = minimum xs
